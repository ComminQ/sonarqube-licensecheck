...
import static at.porscheinformatik.sonarqube.licensecheck.LicenseCheckRulesDefinition.RULE_REPO_KEY_PYTHON;
...
public class LicenseCheckSensor implements Sensor {
  ...
  private static void saveMeasures(SensorContext sensorContext, Set<License> licenses, Set<Dependency> dependencies) {
    ...
    sensorContext.<Integer>newMeasure()
        ...
        .withValue(
            (int) dependencies.stream().filter(d -> Dependency.Status.Forbidden.equals(d.getStatus()))
                .count())
        .on(sensorContext.project())
        .save();
    ...
    sensorContext.<Integer>newMeasure()
        ...
        .withValue(
            (int) dependencies.stream().filter(d -> Dependency.Status.Unknown.equals(d.getStatus()))
                .count())
        .on(sensorContext.project())
        .save();
  }
